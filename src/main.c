...
/*   Updated: 2023/11/27 18:38:29 by jrenau-v         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	fdf_key_hook(int key, t_movement *movement)
{
	if (movement)
		printf("yes\n");
...
int	fdf_key_release_hook(int key, t_movement *movement)
{
	if (movement)
		printf("yes\n");
	printf("The released key is: %d\n", key);
	return (1);	
}

int run_game(void *map)
{
	if (map)
		printf("Have map \n");
	else
		printf("Dont have map \n");
	return (1);
}
...
//	map.player.direction.y = 1;
	...
	mlx_put_image_to_window(minimap.img.mlx_ptr, minimap.img.win_ptr,
			minimap.img.img, 0, 0);
	mlx_hook(minimap.img.win_ptr, 2, 1L << 0, fdf_key_hook, NULL); // This has to be changed to movement
	mlx_hook(minimap.img.win_ptr, 3, 1L << 1, fdf_key_release_hook, NULL); // This has to be changed to movement
//	mlx_loop_hook(img.mlx_ptr, run_game, (void *) &minimap);
